...
def write_prov5_certification(
    *, prov3_report: Path, output_dir: Path, volume_rows: int = 5000
) -> Path:
    if volume_rows < 1:
    ...
    report = {
        ...
        "cloud_database_accessed": False,
        "cloud_database_writes": 0,
        "execution_enabled": False,
        "source_prov3_report": str(prov3_report),
        "clone": {
            "rows_seeded": volume_rows,
            "bytes_before_disposal": clone_bytes,
        ...
        "rollback": {
            "provenance_table_removed": rollback_table_absent,
...
def _seed_volume(path: Path, samples: list[dict[str, Any]], count: int) -> None:
    ...
    for index in range(1, count + 1):
        ...
        forecasts.append(
            (
                index,
                f"{sample.get('ticker')}#CLONE{index}",
...
def _backfill(path: Path) -> dict[str, Any]:
    ...
    for row in connection.execute("SELECT * FROM forecasts ORDER BY id"):
        ...
        raw = {
            ...
            "feature_source_table": table,
            "feature_source_id": source_id,
            "event_at": row["forecasted_at"],
...
def _feature_ref(model: str, feature: dict[str, Any]) -> tuple[str | None, int | None]:
    if model == "crypto_v2":
        return "crypto_features", feature.get("crypto_feature_id")
    if model == "weather_v2":
        return "weather_features", feature.get("weather_feature_id")
    if model == "sports_v1":
        return "sports_features", feature.get("sports_feature_id")
    return None, None
...
def _verify_events(path: Path) -> bool:
    ...
    for row in connection.execute("SELECT * FROM runtime_provenance_events ORDER BY id"):
        ...
        valid = (
            valid and expected == row["provenance_digest"] and row["previous_digest"] == "GENESIS"
        )
    connection.close()
    return valid
