...
def normalize_process_progress(
    process: Mapping[str, Any], *, reference_time: datetime
    ...
    runtime_source = (
        "TIMESTAMPS" if started_at else ("REPORTED" if process.get("runtime") else "UNKNOWN")
    )
    ...
    freshness_age = (
        max(0, int((reference_time - updated_at).total_seconds())) if updated_at else None
    )
    freshness = (
        "STALE"
        if freshness_age is not None and freshness_age > STALE_AFTER_SECONDS
        else ("FRESH" if updated_at else "UNKNOWN")
    )
    ...
    return {
        ...
        "progress_source": "UNITS"
        if derived_percent is not None
        else ("REPORTED" if percent is not None else "UNKNOWN"),
