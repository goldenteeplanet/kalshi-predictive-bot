    ...
    *,
    ...
    for decision in routing.get("decisions") or []:
        ...
        for channel in decision.get("channels") or []:
            if channel not in SIMULATED_LOCAL_CHANNELS:
                diagnostics.append(
                    f"NONLOCAL_CHANNEL_REJECTED:{decision.get('incident_id')}:{channel}"
                )
                continue
            ...
            reason = (
                "NONCRITICAL_RATE_LIMIT"
                if rate_limited
                else "CRITICAL_RATE_LIMIT_BYPASS"
                if active_notification_channel
                and critical
                and (
                    delivered_recent_minute >= per_minute_limit
                    or delivered_recent_hour >= per_hour_limit
    ...
    return {
        ...
        "summary": {
            ...
            "simulated_delivered": sum(
                row["status"] == "SIMULATED_DELIVERED" for row in new_receipts
            ),
            "simulated_rate_limited": sum(
                row["status"] == "SIMULATED_RATE_LIMITED" for row in new_receipts
            ),
            "retained": len(retained),
            "all_critical_deliverable": all(
                row["status"] == "SIMULATED_DELIVERED" for row in critical_receipts
            ),
...
def write_delivery_simulator_preview(
    ...
    *,
    ...
    prior = (
        json.loads(receipts_path.read_text(encoding="utf-8")).get("receipts", [])
