...
def reconcile_notification_receipts(
    ...
    *,
    ...
    for key, decision in sorted(
        expected.items(), key=lambda item: (str(item[0][0]), str(item[0][1]))
    ):
        ...
        latencies = [
            max(0, int((_time(str(row["delivered_at"])) - routed_at).total_seconds()))
        ...
        rows.append(
            {
                ...
                "coverage_passed": len(delivered) == 1
                and len(matches) == 1
                and all(value <= max_latency_seconds for value in latencies),
            }
        )
            ...
            rows.append(
                {
                    "incident_id": key[0],
                    "channel": key[1],
                    "severity": matches[0].get("severity"),
                    "expected": False,
                    "receipt_count": len(matches),
                    "delivered_count": sum(
                        row.get("status") == "SIMULATED_DELIVERED" for row in matches
                    ),
                    "rate_limited_count": sum(
                        row.get("status") == "SIMULATED_RATE_LIMITED" for row in matches
                    ),
                    "latency_seconds": [],
    ...
    return {
        ...
        "summary": {
            ...
            "reconciliation_passed": not any((missing, duplicate, late, unexpected, rate_limited))
...
def write_notification_receipt_audit(
    routing_path: Path, delivery_path: Path, output_dir: Path
) -> Path:
