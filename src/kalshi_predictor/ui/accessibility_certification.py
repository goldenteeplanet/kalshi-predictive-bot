...
def build_accessibility_certification(project_root: Path) -> dict[str, Any]:
    template = (
        project_root / "src/kalshi_predictor/ui/templates/progress_dashboard.html"
    ).read_text(encoding="utf-8")
    base = (project_root / "src/kalshi_predictor/ui/templates/base.html").read_text(
        encoding="utf-8"
    )
    ...
    checks = {
        "skip_link_and_focus_target": 'class="skip-link"' in base
        and 'id="main-content" tabindex="-1"' in base,
        "semantic_page_heading": 'aria-labelledby="progress-page-title"' in template
        and 'id="progress-page-title"' in template,
        ...
        "keyboard_poll_toggle": "data-progress-poll-toggle" in template
        and 'aria-pressed="false"' in template,
        "polling_busy_state": 'aria-busy="false"' in template and 'setAttribute("aria-busy"' in js,
        "polling_pause_resume": "Resume automatic updates" in js
        and "Pause automatic updates" in js,
        ...
        "responsive_single_column": "@media (max-width:700px)" in css
        and ".progress-summary-grid,.workstream-grid{grid-template-columns:1fr}" in css,
        "mobile_table_overflow": ".table-wrap table{min-width:680px}" in css
        ...
        "status_not_color_only": "border:1px solid currentColor" in css
        and all(
            state in template for state in ("RUNNING", "WAITING", "BLOCKED", "PASSED", "FAILED")
        )
        is False,
        "status_contrast": all(value >= 4.5 for value in ratios.values()),
    }
    # State words are rendered from data, while border plus visible text prevents color-only
    # signaling.
    checks["status_not_color_only"] = (
        "border:1px solid currentColor" in css and 'data-progress-field="process_state"' in template
    )
    return {
        "phase": "UI-OBS-2K",
