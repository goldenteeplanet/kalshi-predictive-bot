...
def build_remediation_roadmap(
    objective_path: Path, readiness_path: Path, liquidity_path: Path
) -> dict[str, Any]:
    ...
    open_phases = [
        row
        for row in phases
        if row.get("status") in {"WAITING", "BLOCKED", "APPROVAL_REQUIRED", "RUNNING"}
    ]
    ...
    remediation = [
        {
            "order": 1,
            "phase": "PROV-14B",
            "reason": "Produce exact future observation and market-snap"
            "shot attribution references.",
            "activation_allowed": False,
        },
        {
            "order": 2,
            "phase": "PROV-14C/14D",
            "reason": "Prove multi-cycle attribution stability before scheduler integration.",
            "activation_allowed": False,
        },
        {
            "order": 3,
            "phase": "PMB-35A/35",
            "reason": "Supply the exact missing weather reference proba"
            "bility; never synthesize "
            "defaults.",
            "activation_allowed": False,
        },
        {
            "order": 4,
            "phase": "PMB-36",
            "reason": "Collect disabled-shadow comparisons only after exact compatibility passes.",
            "activation_allowed": False,
        },
        {
            "order": 5,
            "phase": "READINESS-1 recheck",
            "reason": "Recompute unchanged edge, liquidity, spread, risk, and paper gates from "
            "fresh evidence.",
            "activation_allowed": False,
        },
    ]
