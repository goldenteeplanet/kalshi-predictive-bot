...
def write_prov6a_preview(*, evidence: dict[str, Any], output_dir: Path) -> Path:
    ...
    indexes = {
        row["name"]: row.get("columns", []) for row in lineage.get("market_legs_indexes", [])
    }
    schema_equivalent = (
        ...
        and any(
            row.get("unique") == 1 and row.get("columns") == ["ticker", "leg_index"]
    ...
    gates = {
        ...
        "backup_capacity_sufficient": int(storage.get("backup_available_bytes") or 0)
        >= int(storage["minimum_required_bytes"]),
    ...
    report = {
        ...
        "volume_attached_or_mounted": False,
        "alembic_stamped_or_upgraded": False,
        ...
        "lineage_assessment": {
            ...
            "classification": (
                "SCHEMA_PRESENT_MARKER_STALE"
                if schema_equivalent and not revision_matches
                else "LINEAGE_REQUIRES_FURTHER_REVIEW"
            ),
            "extra_safe_indexes": sorted(
                set(indexes) - {"ix_market_legs_category_ticker", "sqlite_autoindex_market_legs_1"}
            ),
        },
        "storage_assessment": {
        ...
        "reconciliation_plan": {
            ...
            "commands_after_separate_approval": [
                ...
                "# Format and mount only the verified new empty device, then persist its "
                "UUID in /etc/fstab.",
                "mountpoint -q /mnt/kalshi-backup",
                'test "$(df -B1 --output=avail /mnt/kalshi-backup | tail -1)" -ge 25411691520',
                "# Create and integrity-check the PROV-6 binary backup using the "
                "commands in the PROV-6 report.",
                ...
                "RUNTIME_PROVENANCE_DUAL_WRITE_ENABLED=false .venv/bin/alembic upgrade "
                "20260624_0011",
                "test \"$(sqlite3 -readonly /var/lib/kalshi-bot/kalshi_phase1.db 'SELECT "
                "version_num FROM alembic_version;')\" = '20260624_0011'",
            ],
        },
        "gates": gates,
        "summary": {
