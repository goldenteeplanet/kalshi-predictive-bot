...
SSH_CONFIG_TOKEN = "I_APPROVE_R39_SSH_CONFIG"  # pragma: allowlist secret
...
def _parse_probe_outputs(results: list[RemoteProbeResult]) -> dict[str, Any]:
    ...
    return {
        "ssh_batch_login_ok": bool(
            by_name.get("ssh_batch_login") and by_name["ssh_batch_login"].ok
        ),
...
def _render_ssh_config_handoff(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "fi",
        "",
        ': "${HOME:?Set HOME before applying SSH configuration}"',
...
def _render_root_bootstrap(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        (
            "echo '[phase3bb-r39] running helper once to install scheduler files and enable "
            "timer without starting'"
        ),
        '"$HELPER"',
...
def _render_executive_summary(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            (
                "- Future scheduler install can use a root-installed helper instead of "
                "prompting for sudo."
            ),
...
def _render_next_actions(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "```bash",
            (
                "PHASE3BB_R38_CODE_SYNC=I_APPROVE_R38_CODE_SYNC bash "
