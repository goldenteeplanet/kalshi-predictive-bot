"""Separate half-hour-grid Miami research version; no execution authority."""
...
def forecast_miami_prior_day_grid30(
    captures: Sequence[MiamiIndexCapture], *, origin_at: datetime,
    model_input_as_of: datetime, horizon_minutes: int,
) -> dict[str, Any]:
    """Separate research version matching prior-day endpoints to exact origin minute."""
    return _forecast_miami_prior_day(
        captures, origin_at=origin_at, model_input_as_of=model_input_as_of,
        horizon_minutes=horizon_minutes, half_hour=True,
    )


def _forecast_miami_prior_day(
    ...
    *,
    ...
    if half_hour:
        if local_origin.minute not in (0, 30) or local_origin.second or local_origin.microsecond:
            raise ValueError("MIAMI_EXACT_HALF_HOUR_GRID_REQUIRED")
    elif local_origin.minute or local_origin.second or local_origin.microsecond:
    ...
    for day in sorted({at.astimezone(LOCAL).date() for at in points}):
        if day >= local_origin.date():
            continue
        start = datetime(
            day.year, day.month, day.day, local_origin.hour, local_origin.minute, tzinfo=LOCAL
        )
    ...
    return {
        "schema": "miami-prior-day-grid30-v1" if half_hour else "miami-prior-day-prospective-v1",
        "model": (
            "miami_prior_day_increment_grid30_v1" if half_hour else "miami_prior_day_increment_v1"
        ),
