...
def preview_liquidity_score(
    *, volume: Any, open_interest: Any, market_liquidity: Any, orderbook_depth_notional: Decimal
) -> dict[str, str]:
    existing = max(to_decimal(market_liquidity) or Decimal("0"), Decimal("0"))
    proposed_input = max(existing, orderbook_depth_notional)
    return {
        "current_score": str(
            score_liquidity(volume=volume, open_interest=open_interest, liquidity=existing)
        ),
        ...
        "preview_score": str(
            score_liquidity(volume=volume, open_interest=open_interest, liquidity=proposed_input)
        ),
    }


def write_gh1k_report(
    *,
    gh1j_report: Path,
    database_path: Path,
    output_dir: Path,
    ...
    weather = [
        row
        for row in audit["ticker_audits"]
        if row["category"] == "weather"
        and row["calibration_ranking_advance"]
        and row["first_funnel_break"] == "LOCAL_SNAPSHOT_MISSING"
    ]
        ...
        with httpx.Client(base_url=rest_base_url, timeout=15.0) as client:
            for item in crypto:
                ...
                snapshot = connection.execute(
                    "SELECT * FROM market_snapshots WHERE ticker=? ORDER BY captured_at "
                    "DESC LIMIT 1",
                    (ticker,),
                ).fetchone()
                ranking = connection.execute(
                    "SELECT * FROM market_rankings WHERE ticker=? ORDER BY ranked_at DESC LIMIT 1",
                    (ticker,),
                ).fetchone()
                ...
                repairs.append(
                    {
                        "ticker": ticker,
                        "stored_ranking_liquidity_score": ranking["liquidity_score"]
                        if ranking
                        else None,
                        **scoring,
                        "trace": "Ranking liquidity uses market volume, open interest, and "
                        "liquidity_dollars; orderbook depth is not an input.",
                        "repair_action": "PREVIEW_ONLY_USE_MAX_MARKET_LIQUIDITY_OR_TOP5_OR"
                        "DERBOOK_NOTIONAL",
    ...
    report = {
        ...
        "mode": "FILESYSTEM_PREVIEW_ONLY",
        "execution_enabled": False,
        "database_writes": 0,
        "trace_result": "ORDERBOOK_DEPTH_NOT_CURRENTLY_USED_BY_RANKING_LIQUIDITY_SCORE",
        "crypto_repair_preview": repairs,
        "weather_staged_files": staged,
        "summary": {
            "crypto_rows_previewed": len(repairs),
            "weather_rows_staged": len(staged),
            "crypto_scores_changed_in_preview": sum(
                row["current_score"] != row["preview_score"] for row in repairs
            ),
        },
    }
