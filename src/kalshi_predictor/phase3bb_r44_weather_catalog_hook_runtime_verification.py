    ...
    *,
    ...
    return {
        ...
        "r40_scheduler_job_runs_available": bool(
            isinstance(r40_payload, dict)
            and isinstance(r40_payload.get("scheduler_job_runs"), list)
        ),
        ...
        "scheduler_failed_log_count": len(
            re.findall(r"\bfailed\b|Failed with result", journal, flags=re.IGNORECASE)
        ),
        "scheduler_traceback_count": len(
            re.findall(r"\bTraceback \(most recent call last\):", journal)
        ),
    }
...
def _latest_weather_catalog_sequence(job_events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    preview_after_hook = _has_event(
        "WEATHER_CATALOG_PREVIEW_WRITTEN", after=hook_index, before=fast_index
    )
    ...
    complete = bool(
        sync_after_hook and parse_after_hook and preview_after_hook and fast_index is not None
    )
    return {
...
def _runtime_checks(parsed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _check(
            ...
            bool(parsed.get("runner_hook_present")),
            f"hook_present={parsed.get('runner_hook_present')}.",
        ),
        _check(
            "runner_hook_before_weather_fast_lane",
            bool(parsed.get("runner_hook_before_fast_lane")),
            f"hook_before_fast_lane={parsed.get('runner_hook_before_fast_lane')}.",
        ),
        _check(
            "weather_catalog_hook_run_seen",
            int(parsed.get("weather_catalog_hook_run_count") or 0) > 0,
            f"runs={parsed.get('weather_catalog_hook_run_count')}.",
        ),
        _check(
            "weather_catalog_sync_seen",
            bool(sequence.get("sync_after_hook")),
            f"sequence={sequence.get('status')}.",
        ),
        _check(
            "weather_catalog_parse_seen",
            bool(sequence.get("parse_after_hook")),
            f"sequence={sequence.get('status')}.",
        ),
        _check(
            "weather_catalog_preview_seen",
            bool(sequence.get("preview_after_hook")),
            f"sequence={sequence.get('status')}.",
        ),
        _check(
            "weather_fast_lane_after_catalog",
            bool(sequence.get("fast_lane_after_hook")),
            f"sequence={sequence.get('status')}.",
        ),
        _check(
            "weather_fast_lane_funnel_written",
            bool(sequence.get("weather_funnel_after_fast_lane")),
            f"sequence={sequence.get('status')}.",
        ),
        _check(
            "weather_activation_preview_json_ok",
            bool(parsed.get("weather_activation_preview_json_ok")),
            "R12 preview JSON exists and parses.",
        ),
        _check(
            "weather_funnel_json_ok",
            bool(parsed.get("weather_funnel_json_ok")),
            "Weather fast-lane funnel JSON exists and parses.",
        ),
        _check(
            "weather_reports_present",
            int(parsed.get("weather_catalog_present_report_count") or 0) >= 4,
            f"present={parsed.get('weather_catalog_present_report_count')}.",
        ),
        _check(
            "r40_understands_scheduler_job_runs",
            bool(parsed.get("r40_scheduler_job_runs_available")),
            "R40 JSON exposes scheduler_job_runs.",
        ),
        _check(
        ...
        _check(
            ...
            "R40, R44, R12 preview, and weather fast-lane help are registered.",
        ),
        _check(
            "scheduler_no_tracebacks",
            int(parsed.get("scheduler_traceback_count") or 0) == 0,
            f"tracebacks={parsed.get('scheduler_traceback_count')}.",
        ),
    ]
...
def _decision(checks: list[dict[str, Any]], parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    if failed:
        if failed[0]["check"].startswith("r40_"):
            ...
            reason = (
                f"Runtime hook evidence exists, but R40 still needs refresh: {failed[0]['check']}."
            )
        else:
    ...
    return {
        ...
        "weather_activation_rows_safe_to_relink": parsed.get(
            "weather_activation_rows_safe_to_relink"
        ),
        "weather_funnel_status": parsed.get("weather_funnel_status"),
        "r40_weather_catalog_hook_job_run_count": parsed.get(
            "r40_weather_catalog_hook_job_run_count"
        ),
...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        ["", "## Scheduler Job Events", "", "| Event | Job | Count | Line |", "|---|---|---:|---|"]
    )
