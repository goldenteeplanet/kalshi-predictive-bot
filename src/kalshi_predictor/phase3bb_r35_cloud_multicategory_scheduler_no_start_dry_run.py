...
def _render_service_draft() -> str:
    return "\n".join(
        [
            ...
            "ExecStart=/opt/kalshi-predictive-bot/scripts/kalshi-multicategory-refresh-runner.sh",
            "TimeoutStartSec=900",
            "",
        ]
    )
...
def _render_runner_draft(jobs: list[dict[str, Any]]) -> str:
    lines = [
        ...
        (
            'print("[phase3bb-r35] db-writer-monitor JSON parse failed; skip writer-gated '
            'job", file=sys.stderr)'
        ),
        "raise SystemExit(1)'",
        "}",
