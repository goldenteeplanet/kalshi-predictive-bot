...
def write_independent_domain_experiment(
    *, database_url: str, output_dir: Path = Path("reports/independent_domain_experiment")
    ...
    _write_text(
        output_dir / "INDEPENDENT_DOMAIN_EXPERIMENT.md",
        _experiment_markdown(selected, evidence[selected], summary),
    )
    ...
    for domain, (link_model, feature_model, source_model) in specs.items():
        ...
        settled_count = (
            session.scalar(
                ...
                .where(Forecast.model_name.in_(models))
            )
            or 0
        )
        ...
        snapshot_count = (
            session.scalar(
                select(func.count(MarketSnapshot.id)).where(MarketSnapshot.ticker.in_(tickers))
            )
            or 0
        )
        result[domain] = {
            ...
            "source_rows": _count(session, source_model)
            if source_model is not None
            else _count(session, WeatherFeature),
...
def rank_domains(evidence: dict[str, dict[str, int]]) -> list[dict[str, Any]]:
    ...
    for domain, base in BASE_SCORES.items():
        ...
        ranked.append(
            {
                "domain": domain,
                "score": float(weighted.quantize(Decimal("0.001"))),
                "component_scores": {k: int(v) for k, v in scores.items()},
...
def summarize_ledger(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    market_brier = [
        Decimal(str(row["market_brier"])) for row in rows if row["market_brier"] is not None
    ]
    pnl = [
        Decimal(row["one_contract_pnl_after_fee"])
        for row in rows
        if row["one_contract_pnl_after_fee"] is not None
    ]
    return {
        ...
        "outperforms_market": bool(
            model_brier
            and market_brier
            and (sum(model_brier) / len(model_brier)) < (sum(market_brier) / len(market_brier))
        ),
...
def build_readiness(domain: str, summary: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    evidence_pass = (
        summary["settled_observations"] >= 100 and summary["no_lookahead_violations"] == 0
    )
    performance_pass = summary["outperforms_market"] and summary["positive_post_cost"]
    return {
        "domain": domain,
        "phase7_evidence_pass": evidence_pass,
        "phase7_performance_pass": performance_pass,
...
def _executable_decision(
    probability: Decimal, snapshot: MarketSnapshot | None, forecast_ask: Decimal | None
...
def _ranking_markdown(ranking: list[dict[str, Any]], selected: str) -> str:
    lines = [
        "# Phase 7 Domain Ranking",
        "",
        f"Selected domain: `{selected}`",
        "",
        "| Rank | Domain | Score | Links | Features | Settled forecasts |",
        "|---:|---|---:|---:|---:|---:|",
    ]
    for index, row in enumerate(ranking, 1):
        ev = row["canonical_evidence"]
        lines.append(
            f"| {index} | {row['domain']} | {row['score']:.3f} | {ev['links']} | {ev['features']} | {ev['settled_forecasts']} |"
        )
    lines.extend(
        [
            "",
            "Scores combine external-data quality, independence, settlement cadence, history, liquidity, executable books, modelability, licensing, and canonical evidence.",
        ]
    )
    return "\n".join(lines) + "\n"
