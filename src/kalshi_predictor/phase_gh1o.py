...
def evaluate_independent_forecast(
    *,
    forecast: Mapping[str, Any],
    market: Mapping[str, Any],
    orderbook: Mapping[str, Any],
    settings: Settings,
    max_forecast_age_minutes: Decimal,
) -> dict[str, Any]:
    now = utc_now()
    ...
    yes_edge = (
        probability - book.best_yes_ask
        if probability is not None and book.best_yes_ask is not None
        else None
    )
    no_probability = Decimal("1") - probability if probability is not None else None
    no_edge = (
        no_probability - book.best_no_ask
        if no_probability is not None and book.best_no_ask is not None
        else None
    )
    ...
    time_score = score_time_to_close(
        time_minutes, min_minutes=settings.opportunity_min_time_to_close_minutes
    )
    ...
    return {
        ...
        "forecast_age_minutes": _string(age),
        "yes_probability": _string(probability),
        "best_side": side,
        "best_price": _string(price),
        "executable_edge": _string(edge),
        "expected_value": _string(metrics.expected_value),
        "opportunity_score": str(metrics.payout_adjusted_score),
        "spread": _string(book.spread),
        "liquidity_score": str(liquidity_score),
        "top5_notional": str(top5_notional),
        "time_to_close_minutes": _string(time_minutes),
        "blockers": blockers,
...
def write_gh1o_report(
    *,
        ...
        with httpx.Client(base_url=rest_base_url, timeout=15.0) as client:
            for stored in forecasts:
                ...
                if market_response.status_code != 200:
                    skips["PUBLIC_MARKET_UNAVAILABLE"] = (
                        skips.get("PUBLIC_MARKET_UNAVAILABLE", 0) + 1
                    )
                    continue
                ...
                if book_response.status_code != 200:
                    ...
                    continue
                rows.append(
                    evaluate_independent_forecast(
                        forecast=dict(stored),
                        market=market,
                        orderbook=book_response.json(),
                        settings=settings,
                        max_forecast_age_minutes=max_forecast_age_minutes,
                    )
                )
    finally:
        connection.close()
    report = {
        ...
        "mode": "INDEPENDENT_FORECAST_PUBLIC_BOOK_READ_ONLY",
        ...
        "evaluations": rows,
        "summary": {
            "stored_forecasts_scanned": len(forecasts),
            "live_markets_evaluated": len(rows),
            "positive_executable_edge": sum(
                (to_decimal(row["executable_edge"]) or Decimal("0")) > 0 for row in rows
            ),
