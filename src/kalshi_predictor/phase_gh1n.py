...
def synchronized_market_implied_audit(
    *, ticker: str, payload: Mapping[str, Any], fetch_started_at: str, fetch_completed_at: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "midpoint_forecast": _string(midpoint),
        "yes_ask": _string(yes_ask),
        "no_ask": _string(no_ask),
        "yes_executable_edge": _string(yes_edge),
        "no_executable_edge": _string(no_edge),
        "best_executable_edge": _string(best_edge),
        "synchronized_edge_positive": bool(best_edge is not None and best_edge > 0),
        "explanation": "Market-implied midpoint compared with an executa"
        "ble ask includes half-spread "
        "cost.",
    }


def write_gh1n_report(
    *,
    gh1m_report: Path,
    database_path: Path,
    output_dir: Path,
        ...
        with httpx.Client(base_url=rest_base_url, timeout=15.0) as client:
            for ticker in tickers:
                ...
                stored = connection.execute(
                    ...
                    "AND f.model_name='market_implied_v1' ORDER BY f.forecasted_at DESC, "
                ...
                row["stored_forecast_snapshot_skew_seconds"] = (
                    _skew(stored[0], stored[1]) if stored else None
                )
    ...
    report = {
        ...
        "mode": "SYNCHRONIZED_PUBLIC_REST_READ_ONLY",
        ...
        "summary": {
            ...
            "positive_synchronized_edges": positive,
            "nonpositive_synchronized_edges": len(rows) - positive,
            "candidate_set_status": "CLOSED_NEGATIVE_EDGE"
            if positive == 0
            else "KEEP_OPEN_REVIEW_POSITIVE_EDGE",
            "next_action": "RETURN_TO_BOUNDED_DISCOVERY"
            if positive == 0
            else "REVIEW_POSITIVE_SYNCHRONIZED_EDGE",
        },
    }
...
def _skew(forecasted_at: str, captured_at: str) -> str | None:
    try:
        return str(
            abs(
                (
                    datetime.fromisoformat(forecasted_at) - datetime.fromisoformat(captured_at)
                ).total_seconds()
            )
        )
