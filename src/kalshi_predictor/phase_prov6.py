...
def write_prov6_preflight(
    *, evidence: dict[str, Any], prov5_report: Path, output_dir: Path
) -> Path:
    ...
    report = {
        ...
        "summary": {
            ...
            "next_action": (
                "STOP_FOR_EXPLICIT_APPROVAL" if ready else "REMEDIATE_PREFLIGHT_BLOCKERS_AND_RERUN"
            ),
        },
    }
...
def _commands(required_bytes: int) -> dict[str, list[str]]:
    return {
        "preflight": [
            ...
            f"test \"$(sqlite3 -readonly {DATABASE} 'SELECT ve"
            f"rsion_num FROM alembic_version LIMIT 1;')\" = '"
            f"{EXPECTED_PARENT_REVISION}'",
        ...
        "migration_after_separate_approval": [
            ...
            f'sqlite3 -readonly {DATABASE} "SELECT name FROM '
            f"sqlite_master WHERE name='runtime_provenance_eve"
            f"nts';\"",
        ],
        "rollback": [
