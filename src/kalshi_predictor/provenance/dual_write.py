...
def _append_event(
    ...
    *,
    ...
    previous = (
        session.scalar(
            select(RuntimeProvenanceEvent.provenance_digest)
            .where(RuntimeProvenanceEvent.forecast_id == forecast_id)
    ...
    event = RuntimeProvenanceEvent(
        ...
        source_observation_ref_json=(
            json.dumps(observation, sort_keys=True) if observation else None
        ),
        feature_source_table=feature_table,
        feature_source_id=feature_id,
