...
def build_offline_ci_gate(*, bundle_path: Path, manifest_path: Path, root: Path) -> dict[str, Any]:
    bundle = _json_object(bundle_path)
    verification = verify_offline_certification_bundle(bundle_path, root=root)
    expected_manifest = [f"{row['sha256']}  {row['path']}" for row in bundle.get("artifacts", [])]
...
def write_offline_ci_gate(
    *, bundle_path: Path, manifest_path: Path, root: Path, output_path: Path
    ...
    report = build_offline_ci_gate(bundle_path=bundle_path, manifest_path=manifest_path, root=root)
