...
def attribute_ranking(row: Mapping[str, Any], *, settings: Settings) -> dict[str, Any]:
    metrics = calculate_payout_metrics(
        ...
        yes_probability=row.get("forecast_probability"),
        cost=row.get("best_price"),
        edge=row.get("estimated_edge"),
        liquidity_score=row.get("liquidity_score"),
        spread_score=row.get("spread_score"),
        confidence_score=row.get("model_confidence_score"),
...
def write_gh1m_report(
    *, gh1j_report: Path, database_path: Path, settings: Settings, output_dir: Path
) -> Path:
    audit = json.loads(gh1j_report.read_text(encoding="utf-8"))
    tickers = [
        row["ticker"] for row in audit["ticker_audits"] if row["calibration_ranking_advance"]
    ]
    ...
    report = {
        ...
        "mode": "SQLITE_READ_ONLY_GATE_RECHECK",
        ...
        "summary": {
            "tickers_attributed": len(attributed),
