...
def _run_nyc_w9_cycle(
    *,
    ...
    w8_path = write_nyc_w8_report(reports_dir=reports_dir, output_dir=reports_dir / "phase_nyc_w8")
    ...
    active_kalshi = kalshi_client or httpx.Client(base_url=PRODUCTION_PUBLIC_REST_URL, timeout=15.0)
    try:
        ...
        certification_path = write_nyc_w3_report(
            ...
            user_agent=user_agent,
            market_limit=len(state["pinned_tickers"]),
            tolerance_minutes=15,
            exact_tickers=list(state["pinned_tickers"]),
        ...
        write_shadow_runtime_report(
            reports_dir=reports_dir,
            output_dir=reports_dir / f"phase_nyc_w7_live_{slug}",
            max_adjustment=max_adjustment,
            source_paths=[preview_path],
        )
...
def _discover_next_ladder(
    ...
    for market in response.json().get("markets", []):
        ...
        if contract is None or contract.target_utc_time < now + MIN_PIN_LEAD:
            continue
        target = contract.target_utc_time.astimezone(UTC)
    ...
    if path.exists():
        try:
            ...
            raise StateIntegrityError(
                "NYC-W9 state is unreadable; pinned state was not reset"
            ) from exc
...
def _write_cycle_report(
    ...
    report = {
        ...
        "mode": "EXACT_LIVE_WINDOW_READ_ONLY_FEED",
        "status": status,
        "database_writes": 0,
