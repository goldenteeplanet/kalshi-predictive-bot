...
def run_atomic_activation(
    *,
    ...
    minimum_close_minutes: int = 0,
    immediate_edge_evaluation: bool = False,
    ...
    result: dict[str, Any] = {
        ...
        "backup_path": str(verified_backup),
        "pinned_tickers": {},
    ...
    with session_factory() as session:
        try:
            ...
            if (
                linked.ambiguous_markets
                or linked.unsupported_markets
            ...
            for market, _ in pinned["weather_v2"]:
                ...
                insert_weather_market_link(
                    ...
                    target_value=contract.discrete_threshold_f,
                    target_time=contract.target_utc_time,
                    confidence="1.0",
                    reason="GH-1T atomic exact metadata validation",
            ...
            if (
                session.scalar(
                    select(CryptoFeature.id)
                    .where(CryptoFeature.symbol == "BTC")
                    .order_by(CryptoFeature.generated_at.desc())
                    .limit(1)
                )
                is None
            ):
                ...
                try:
                    for snapshot in model_snapshots:
                        ...
                        if forecast is None:
                            ...
                            insert_forecast(session, forecast, market_snapshot_id=snapshot.id)
...
def _fetch_pinned(
    ...
    with httpx.Client(base_url=base_url, timeout=15.0) as client:
        for model, series_group in (
            ("crypto_v2", DEFAULT_CRYPTO_SERIES),
            ("weather_v2", ("KXTEMPNYCH",)),
        ):
            for series in series_group:
                if len(selected[model]) >= bound:
                    break
                response = client.get(
                    "/markets", params={"limit": 100, "status": "open", "series_ticker": series}
                )
                ...
                eligible = [
                    ...
                    if _has_required_lead_time(market, minimum_close_minutes=minimum_close_minutes)
                ]
                if not eligible:
                    continue
                ...
                window = [
                    ...
                    if parse_datetime(market.get("close_time")) == earliest
                ]
...
def _ensure_exact_weather_feature(session: Session, target: Any) -> int:
    existing = session.scalar(
        ...
        .where(WeatherFeature.location_key == "new_york", WeatherFeature.target_time == target)
        .limit(1)
    )
    ...
    forecast = session.scalar(
        ...
        .where(WeatherForecast.location_key == "new_york", WeatherForecast.forecast_time == target)
        .order_by(WeatherForecast.forecast_generated_at.desc())
        .limit(1)
    )
    if forecast is None:
