...
def build_pmb35a_diagnostics(mapping_path: Path) -> dict[str, Any]:
    ...
    for row in source.get("rows", []):
        ...
        exact_missing = [
            item for item in diagnostics if "FIELD_MISSING" in item or "CONTEXT_MISSING" in item
        ]
        blocked.append(
            {
                ...
                "required_repair": (
                    "Provide an independent exact reference forecast probability and "
                    "distinct current/reference executable-spread snapshots for the same "
                    "ticker/category/time identity."
                ),
    ...
    for row in blocked:
        for diagnostic in row["exact_missing_inputs"]:
            priorities.append(
                {
                    "priority": 1 if "REFERENCE_FORECAST" in diagnostic else 2,
                    "diagnostic": diagnostic,
                    "fixture_id": row["fixture_id"],
                }
            )
    priorities.sort(
        key=lambda item: (item["priority"], item["diagnostic"], str(item["fixture_id"]))
    )
    ...
    return {
        ...
        "source": {
            ...
            "pmb35_deployment_unblocked": source.get("summary", {}).get(
                "pmb35_deployment_unblocked"
            )
            is True,
        },
        ...
        "summary": {
            ...
            "exact_missing_inputs": sum(len(row["exact_missing_inputs"]) for row in blocked),
            "pmb35_deployment_unblocked": not blocked,
        },
