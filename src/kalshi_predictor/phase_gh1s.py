...
def preview_exact_links(
        ...
        for candidate in candidates:
            ...
            if model == "crypto_v2":
                ...
                safe = (
                    ...
                    and confidence >= settings.crypto_v2_min_link_confidence
                )
                rows.append(
                    _row(
                        ticker,
                ...
                rows.append(
                    _row(
                        ...
                        target_time=contract.target_utc_time.isoformat(),
                        confidence="1.0",
                    )
                )
    ...
    return {
        ...
        "mode": "EXACT_LINK_PREVIEW_NO_WRITE",
        "database_writes": 0,
...
def apply_exact_links(
    ...
    *,
    ...
    crypto_result = (
        link_crypto_markets(session, tickers=crypto, limit=len(crypto)) if crypto else None
    )
    weather_written = 0
    for row in weather:
        insert_weather_market_link(
            ...
            weather_metric=row["weather_metric"],
            target_operator=row["target_operator"],
            target_value=row["target_value"],
            target_time=parse_datetime(row["target_time"]),
            confidence=row["confidence"],
            reason="GH-1S exact market metadata validation",
    ...
    return {
        ...
        "crypto_links_written": getattr(crypto_result, "links_created", 0),
        "crypto_links_already_present": getattr(crypto_result, "already_linked", 0),
        "weather_links_written": weather_written,
        "total_safe_rows": len(safe),
        "execution_enabled": False,
    }
