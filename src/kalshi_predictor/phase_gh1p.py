    ...
    *,
    ...
    with httpx.Client(base_url=rest_base_url, timeout=15.0) as client:
        for model, series_group in (
            ...
            ("weather_v2", DEFAULT_WEATHER_SERIES),
        ):
            for series in series_group:
                if len(selected[model]) >= max_markets_per_category:
                    break
                response = client.get(
                    "/markets", params={"limit": 20, "status": "open", "series_ticker": series}
                )
        ...
        for model, payloads in selected.items():
            for market, orderbook in payloads:
                ...
                if ok:
                    ...
                    ineligible[model].append(
                        {"ticker": str(market.get("ticker") or ""), "reason": reason}
                    )
        session.rollback()
