...
checks = {
    "catalog_nonempty": catalog["count"] > 0,
    "bounded": catalog["count"] <= catalog["limit"],
    "hashes_complete": all(len(item["sha256"]) == 64 for item in catalog["items"]),
    "allowlisted_formats": all(
        Path(item["path"]).suffix in catalog["allowed_suffixes"] for item in catalog["items"]
    ),
    "read_only": catalog["read_only"] is True,
}
report = {
    "phase": "UI-OBS-2N",
    "mode": "LOCAL_REPORT_NAVIGATION_AND_EVIDENCE_VIEWER",
