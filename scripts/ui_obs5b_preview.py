...
parser.add_argument(
    "--snapshot-output",
    type=Path,
    default=Path("reports/phase_ui_obs5b/ui_obs5b_browser_snapshot.json"),
)
...
payload = {
    ...
    "r5_recovery9_certification": {
        "status": cert["status"],
        "rollback_verified": cert["gates"]["rollback_hash_verified"],
        "rollback_path": cert["rollback"]["path"],
    },
    "prov14b": {
        "state": "QUEUED",
        "reason": "Waiting for first bounded scheduler cycle and sole-writer isolation.",
    },
}
...
report.update(
    {
        "phase": "UI-OBS-5B",
        "mode": "LOCAL_READ_ONLY_INTEGRATION_PREVIEW",
        "cloud_writes": 0,
        "service_controls": 0,
    }
)
