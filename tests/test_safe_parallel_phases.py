...
def test_ui_obs5b_fails_visible_on_stale_running_heartbeat() -> None:
    phases = [
        {"number": i, "phase": f"P{i}", "status": "PASSED", "evidence": "x"} for i in range(1, 21)
    ]
    payload = {
        ...
        "scheduler": {
            "state": "RUNNING",
            "heartbeat": {"at": "2026-07-19T00:00:00Z", "interval_seconds": 15},
            "legacy_watcher_enabled": False,
            "legacy_watcher_active": False,
        },
        "r5_recovery9_certification": {"status": "PASSED", "rollback_verified": True},
    }
    report = build_live_roadmap_status(
        payload, reference_time=datetime(2026, 7, 19, 0, 1, tzinfo=UTC)
    )
...
def test_pmb35a_never_defaults_missing_weather_reference() -> None:
    report = build_pmb35a_diagnostics(
        Path(
            "tests/fixtures/ci_report_inputs/phase_pmb34a/"
            "pmb34a_exact_shadow_field_source_mapping_preview.json"
        )
    )
...
def test_readiness2b_preserves_disabled_execution() -> None:
    report = build_remediation_roadmap(
        Path(
            "tests/fixtures/ci_report_inputs/phase_objective_status/"
            "objective_20_phase_status_20260719.json"
        ),
        Path(
            "tests/fixtures/ci_report_inputs/phase_readiness2/"
            "readiness2_failed_gate_attribution_fresh_20260719.json"
        ),
        Path("tests/fixtures/ci_report_inputs/phase_gh1x/gh1x_liquidity_edge_risk_census.json"),
    )
