...
def test_pmb34a_maps_exact_real_sources_for_all_categories():
    ...
    assert exact["crypto"]["shadow_context"] == {
        "forecast_bias": "0.004",
        "spread_addition": "0.006",
    }
    assert exact["weather"]["shadow_context"] == {
        "forecast_bias": "-0.008",
        "spread_addition": "0.008",
    }
...
def test_pmb34a_rejects_missing_or_fabricated_defaults():
    ...
    missing = next(
        row for row in fixtures if row["fixture_id"] == "weather-reject-missing-reference"
    )
...
def test_pmb34a_is_deterministic_local_and_disabled(tmp_path):
    first = json.loads(
        write_exact_shadow_field_mapping_preview(FIXTURES, tmp_path / "a").read_text()
    )
    second = json.loads(
        write_exact_shadow_field_mapping_preview(FIXTURES, tmp_path / "b").read_text()
    )
    assert first == second
