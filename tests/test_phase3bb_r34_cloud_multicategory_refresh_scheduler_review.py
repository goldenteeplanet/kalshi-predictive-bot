...
from kalshi_predictor.phase3bb_r34_cloud_multicategory_refresh_scheduler_review import (
    build_phase3bb_r34_cloud_multicategory_refresh_scheduler_review,
    write_phase3bb_r34_cloud_multicategory_refresh_scheduler_review_report,
)

DB_FINGERPRINT = "sha256:r34-test-db"


def test_phase3bb_r34_reviews_scheduler_draft_without_installing(
    tmp_path: Path,
) -> None:
    session_factory = _session_factory(tmp_path)
    reports_dir = tmp_path / "reports"
    _write_context(reports_dir)
    _write_scorecard(reports_dir)

    with session_factory() as session:
        artifacts = write_phase3bb_r34_cloud_multicategory_refresh_scheduler_review_report(
            session,
            output_dir=reports_dir / "phase3bb_r34",
    ...
    jobs = {row["job_id"]: row for row in payload["refresh_jobs"]}
    assert payload["phase"] == "3BB-R34-CLOUD-MULTICATEGORY-REFRESH-SCHEDULER-REVIEW"
    assert decision["status"] == "READY_FOR_NO_START_SCHEDULER_DRY_RUN"
    assert decision["review_passed"] is True
    assert decision["paper_ready_candidates"] == 0
    assert jobs["weather_fast_lane"]["requires_db_writer_gate"] is True
    assert jobs["crypto_background_status"]["command"].startswith("kalshi-bot phase3bc-r5-status")
    assert "phase3bc-r5-unattended-start" not in artifacts.scheduler_draft_path.read_text(
        encoding="utf-8"
    )
    assert all(row["passed"] for row in payload["scheduler_checks"])
...
def test_phase3bb_r34_routes_to_operator_review_when_candidates_exist(
    tmp_path: Path,
) -> None:
    session_factory = _session_factory(tmp_path)
    reports_dir = tmp_path / "reports"
    ...
    decision = payload["scheduler_decision"]
    assert decision["status"] == "PAPER_ONLY_OPERATOR_REVIEW_FIRST"
...
def test_phase3bb_r34_blocks_when_r33_is_not_ready(tmp_path: Path) -> None:
    session_factory = _session_factory(tmp_path)
    reports_dir = tmp_path / "reports"
    _write_context(reports_dir)
    _write_scorecard(reports_dir)
    ...
    decision = payload["scheduler_decision"]
    assert decision["status"] == "BLOCKED_SCHEDULER_REVIEW"
    assert decision["first_failed_check"] == "r33_operations_ready"


def test_phase3bb_r34_cli_help_registered() -> None:
    result = CliRunner().invoke(
        app,
        ["phase3bb-r34-cloud-multicategory-refresh-scheduler-review", "--help"],
    )

    assert result.exit_code == 0
    assert "phase3bb-r34-cloud-multicategory-refresh-scheduler-review" in result.output
    assert "--ui-timeout-seconds" in result.output


def _session_factory(tmp_path: Path):
    get_settings.cache_clear()
    engine = init_db(f"sqlite:///{tmp_path / 'phase3bb_r34.db'}")
...
def _write_scorecard(reports_dir: Path) -> None:
    scorecard_dir = reports_dir / "phase3bb_r3"
    ...
    rows = [
        {
            ...
            "active_markets": "30",
            "linked_markets": "140",
            "top_blocker": "RANKING_MISSING",
            "next_implementation_step": "Run weather fast lane.",
        },
        {
            "category": "economic",
            "score": "97",
            "active_markets": "0",
            "linked_markets": "274",
            "top_blocker": "NO_CURRENT_ACTIVE_MARKETS",
            "next_implementation_step": "Refresh economic parser backlog.",
        },
        {
            "category": "news",
            "score": "70",
            ...
            "linked_markets": "200",
            "top_blocker": "NO_CURRENT_ACTIVE_MARKETS",
            "next_implementation_step": "Discover official news/event sources.",
        },
        {
            "category": "sports",
            "score": "90",
            "active_markets": "45184",
            "linked_markets": "51375",
            "top_blocker": "UNSUPPORTED_KXMVE_COMPOSITES_PARKED",
            "next_implementation_step": "Repair exact sports provenance.",
        },
        {
            "category": "crypto",
            "score": "105",
            "active_markets": "3776",
            "linked_markets": "35028",
            "top_blocker": "ZERO_VISIBLE_DEPTH",
            "next_implementation_step": "Keep crypto as background watcher.",
        },
    ]
    path = scorecard_dir / "category_scorecard.csv"
    ...
    def runner(probe: UiApiProbe, base_url: str) -> UiApiResult:
        if probe.name == "db_writer_api":
            ...
            body = {
                ...
                "dashboard_snapshot_id": "snap-r34-test",
