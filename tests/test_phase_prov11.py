...
def test_prov11_detects_digest_tampering_and_execution_guard(tmp_path, monkeypatch) -> None:
    ...
    with session_factory() as session:
        ...
        session.commit()
        payload = build_provenance_diagnostics(session, event_limit=10, execution_enabled=True)
...
def test_prov12_drift_alerts_detect_stale_and_missing_ranking(tmp_path, monkeypatch) -> None:
    ...
    with session_factory() as session:
        _seed_chain(session)
        ranking_event = (
            session.query(RuntimeProvenanceEvent).filter_by(stage="RANKING_CREATED").one()
        )
...
def _seed_chain(session) -> None:
    now = datetime(2026, 7, 17, 5, 0, tzinfo=UTC)
    session.add(Market(ticker="PROV11", raw_json="{}", first_seen_at=now, last_seen_at=now))
    session.add(
        CryptoFeature(
            id=12,
            symbol="BTC",
