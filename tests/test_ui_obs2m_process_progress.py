...
def test_runtime_progress_and_eta_derive_from_exact_evidence() -> None:
    process, diagnostics = normalize_process_progress(
        {
            "name": "job",
            "pid": 42,
            "state": "RUNNING",
    ...
    process, diagnostics = normalize_process_progress(
        {
            ...
            "state": "RUNNING",
            "updated_at": "2026-07-18T00:10:00Z",
            "estimated_remaining": "about one hour",
        },
        reference_time=REFERENCE,
    )
    assert diagnostics == []
...
def test_stale_running_process_fails_closed() -> None:
    process, diagnostics = normalize_process_progress(
        {"pid": 42, "state": "RUNNING", "updated_at": "2026-07-18T00:00:00Z"},
        reference_time=REFERENCE,
    )
...
def test_completed_process_requires_completed_units_when_units_exist() -> None:
    process, diagnostics = normalize_process_progress(
        {"state": "PASSED", "completed_units": 9, "total_units": 10}, reference_time=REFERENCE
    )
    assert process["state"] == "BLOCKED"
    assert process["estimated_remaining"] == "unknown"
...
def test_eta_above_bound_is_not_displayed() -> None:
    process, _ = normalize_process_progress(
        {
            "pid": 42,
            "state": "RUNNING",
            "started_at": "2026-07-11T00:10:00Z",
            "updated_at": "2026-07-18T00:10:00Z",
            "completed_units": 1,
            "total_units": 100,
        },
