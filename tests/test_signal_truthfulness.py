...
from kalshi_predictor.signals.repository import _signal_card, signal_detail, signal_marketplace
from kalshi_predictor.signals.status import signal_status_rows
from kalshi_predictor.utils.time import utc_now


def test_historical_summary_preserves_measured_zero_values():
    from kalshi_predictor.signals.repository import _research_summary

    signal = SimpleNamespace(signal_name="Measured", status="Average")
    performance = SimpleNamespace(
        raw_json="{}", generated_at=utc_now(), roi=Decimal(0),
        win_rate=Decimal(0), confidence_score=Decimal(0),
    )
    summary = _research_summary(signal, performance)
    assert "ROI is 0, win rate is 0" in summary
    assert "(not calibrated probability) is 0" in summary
...
def test_old_output_cannot_appear_active_and_future_clock_is_unknown(tmp_path):
    ...
    with factory() as session:
        ...
        detail = signal_detail(session, signal_name=signal.signal_name)
        assert detail["card"]["readiness_status"] == "STALE"
        assert detail["card"]["freshness_checked_at"] is not None
