...
from kalshi_predictor.phase3bb_r59_weather_catalog_refresh_r57_retry import (
    build_phase3bb_r59_weather_catalog_refresh_r57_retry,
    write_phase3bb_r59_weather_catalog_refresh_r57_retry_report,
)


def test_phase3bb_r59_waits_refreshes_and_runs_patched_r57(tmp_path: Path) -> None:
    ...
    with session_factory() as session:
        artifacts = write_phase3bb_r59_weather_catalog_refresh_r57_retry_report(
            session,
            output_dir=reports_dir / "phase3bb_r59",
            ...
            probe_runner=_fake_probe_runner(
                calls=calls,
                writer_wait=[
                    _writer_state(safe=False, pid=137766),
                    _writer_state(safe=True, pid=None),
                    _writer_state(safe=True, pid=None),
                ],
                r53_states=[
                    _r53_state(selected=True, forecast_rows=0, ranking_rows=0),
                    _r53_state(selected=True, forecast_rows=0, ranking_rows=0),
                    _r53_state(selected=True, forecast_rows=10, ranking_rows=10, positive_rows=2),
                ],
                paper_gate_summary={
                    "weather_rows": 10,
    ...
    assert payload["decision"]["status"] == "R57_WEATHER_POSITIVE_EV_NOT_PAPER_READY"
    assert payload["decision"]["first_hard_blocker"] == "PHASE_3M_ZERO_SIZE"
    assert names.index("writer_gate_check_1") < names.index("weather_catalog_refresh_parse")
    assert names.index("weather_catalog_refresh_parse") < names.index("weather_per_ticker_forecast")
    assert payload["r57_gate"]["allowed"] is True
    assert payload["safety_flags"]["creates_paper_trades"] is False
    assert artifacts.refresh_steps_csv_path.exists()


def test_phase3bb_r59_does_not_refresh_when_writer_never_clears(tmp_path: Path) -> None:
    session_factory = _session_factory(tmp_path)
    reports_dir = tmp_path / "reports"
    _write_context(reports_dir)
    calls: list[RemoteProbe] = []
    ...
    with session_factory() as session:
        payload = build_phase3bb_r59_weather_catalog_refresh_r57_retry(
            ...
            probe_runner=_fake_probe_runner(
                ...
                r53_states=[_r53_state(selected=True, forecast_rows=0, ranking_rows=0)],
                paper_gate_summary={},
            ),
        )
...
def test_phase3bb_r59_cli_help_registered() -> None:
    result = CliRunner().invoke(
        app,
        ["phase3bb-r59-weather-catalog-refresh-r57-retry", "--help"],
    )

    assert result.exit_code == 0
    assert "phase3bb-r59-weather-catalog-refresh-r57-retry" in result.output
    assert "expected-writer-pid" in result.output


def _session_factory(tmp_path: Path):
    get_settings.cache_clear()
    engine = init_db(f"sqlite:///{tmp_path / 'phase3bb_r59.db'}")
    ...
    *,
    ...
    def _runner(probe: RemoteProbe, target: CloudBootstrapTarget) -> RemoteProbeResult:
        ...
        if probe.name.startswith("writer_gate_check"):
            ...
            stdout = "Synced 100 markets.\nParsed 1500 market leg rows.\n"
...
def _r53_state(
    *,
    ...
    rows = []
    for index in range(10 if selected else 0):
        has_forecast = index < forecast_rows
        has_ranking = index < ranking_rows
        blocker = "FORECAST_MISSING"
        if has_forecast and not has_ranking:
            blocker = "RANKING_MISSING"
        elif has_ranking and index < positive_rows:
    ...
    return {
        ...
        "audit": {
            ...
            "future_series_market_rows": 10 if selected else 0,
            "expired_series_market_rows": 490,
        },
        "summary": {
            "selected_target_time": "2026-07-14T05:00:00+00:00" if selected else None,
            "selected_minutes_until_target": 30 if selected else None,
            "selected_window_market_rows": 10 if selected else 0,
            "selected_window_linked_rows": 10 if selected else 0,
            "selected_window_missing_link_rows": 0,
            "selected_window_stale_link_rows": 0,
            "selected_window_snapshot_rows": 10 if selected else 0,
            "selected_window_fresh_snapshot_rows": 10 if selected else 0,
            "selected_window_source_forecast_rows": 10 if selected else 0,
            "selected_window_feature_rows": 10 if selected else 0,
