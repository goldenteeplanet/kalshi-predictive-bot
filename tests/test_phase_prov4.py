...
def _forecast_payload() -> dict:
    return {
        "ticker": "SYN",
        "forecasted_at": datetime(2026, 7, 16, 16, 2, tzinfo=UTC),
...
def test_prov4_enabled_dual_write_is_append_only_and_hash_chained(
    ...
    for table in (
        ...
        RuntimeProvenanceEvent.__table__,
    ):
        table.create(engine, checkfirst=True)
    with Session(engine) as session:
        _market(session)
        snapshot = MarketSnapshot(
            ticker="SYN", captured_at=datetime(2026, 7, 16, 16, 2, tzinfo=UTC), raw_market_json="{}"
        )
        ...
        ranking = insert_market_ranking(
            session,
            {
                ...
                "market_snapshot_id": snapshot.id,
                "ranked_at": datetime(2026, 7, 16, 16, 3, tzinfo=UTC),
                "raw_json": {},
            },
            attribution_enabled=True,
        )
        ...
        session.commit()
        events = list(
            session.scalars(select(RuntimeProvenanceEvent).order_by(RuntimeProvenanceEvent.id))
        )
