...
def _reports(root: Path, count: int = 25) -> None:
    directory = root / "ui_obs_load"
    directory.mkdir(parents=True)
    for index in range(count):
        (directory / f"report-{index:03}.json").write_text(
            json.dumps({"phase": f"LOAD-{index}", "status": "PASSED"}), encoding="utf-8"
        )
...
def test_large_history_is_bounded_to_twenty_visible_entries(tmp_path: Path) -> None:
    entries = [
        {
            "generated_at": f"2026-07-18T00:{index % 60:02}:00Z",
            "process": {},
            "writer": {},
            "scheduler": {},
            "incidents": [],
...
def test_cached_catalog_has_bounded_memory_and_warm_latency(tmp_path: Path) -> None:
    ...
    for _ in range(500):
        get_cached_evidence_catalog(tmp_path)
...
def test_concurrent_read_only_api_sessions_share_cached_catalog(
    ...
    assert all(
        response.status_code == 200 and response.json()["count"] == 5 for response in responses
    )
