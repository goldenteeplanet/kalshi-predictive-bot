...
def _safe_report(root: Path) -> Path:
    path = root / "ui_obs_test/report.json"
...
def test_non_allowlisted_oversized_and_invalid_ids_fail_closed(tmp_path: Path) -> None:
    _safe_report(tmp_path)
    bad = tmp_path / "other/report.json"
    bad.parent.mkdir()
    bad.write_text("{}")
    huge = tmp_path / "ui_obs_test/huge.json"
...
def test_symlink_is_rejected(tmp_path: Path) -> None:
    ...
    outside.write_text("{}")
    directory = tmp_path / "ui_obs_test"
    directory.mkdir()
...
def test_malformed_json_is_visible_as_invalid_not_executed(tmp_path: Path) -> None:
    path = tmp_path / "ui_obs_test/bad.json"
    path.parent.mkdir()
    path.write_text("{bad")
    catalog = build_evidence_catalog(tmp_path)
