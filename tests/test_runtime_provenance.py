...
def _database(path: Path, *, complete: bool = True) -> None:
    ...
    connection.execute(
        "INSERT INTO forecasts VALUES (1,'TICKER','2026-07-16T16:02:00+00:00',"
        "'weather_v2','0.60',?)",
        (feature,),
    )
    connection.execute(
    ...
    connection.execute(
        "INSERT INTO market_snapshots VALUES (3,'TICKER','2026-07-16T16:03:00+00:00')"
    )
    if complete:
        connection.execute(
            "INSERT INTO features VALUES (4,'TICKER','weather_exact','2026-07-16T16:01:00+00:00')"
        )
    connection.commit()
    connection.close()
...
def test_prov2_reports_missing_runtime_attribution_without_fabricating_it(tmp_path: Path) -> None:
    ...
    report = json.loads(
        write_runtime_provenance_audit(
