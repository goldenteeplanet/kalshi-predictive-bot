...
def _decision(incident_id, severity="WARNING", channels=None, action="DELIVER_NOW"):
    return {
        ...
        "severity": severity,
        "channels": channels or ["local_desktop"],
        "action": action,
    }
...
def test_ui_obs2f_simulates_local_delivery_without_side_effects():
    report = simulate_local_delivery(
        {"decisions": [_decision("one", channels=["local_desktop", "dashboard"])]}, delivered_at=NOW
    )
    assert report["summary"]["simulated_delivered"] == 2
    assert all(
        row["simulated"] and not row["external_side_effect"] for row in report["new_receipts"]
    )
...
def test_ui_obs2f_enforces_per_minute_and_hour_limits_for_noncritical():
    ...
    report = simulate_local_delivery(
        {"decisions": [_decision("minute-limited")]}, delivered_at=NOW, prior_receipts=minute_prior
    )
    ...
    report = simulate_local_delivery(
        {"decisions": [_decision("hour-limited")]}, delivered_at=NOW, prior_receipts=hour_prior
    )
    assert report["new_receipts"][0]["status"] == "SIMULATED_RATE_LIMITED"
...
def test_ui_obs2f_critical_remains_deliverable_when_limits_are_saturated():
    ...
    report = simulate_local_delivery(
        {
            "decisions": [
                _decision("critical", "CRITICAL", ["local_audible", "local_desktop", "dashboard"])
            ]
        },
        delivered_at=NOW,
...
def test_ui_obs2f_retains_only_bounded_newest_receipts():
    ...
    report = simulate_local_delivery(
        {"decisions": [_decision("new", channels=["dashboard"])]},
        delivered_at=NOW,
        prior_receipts=prior,
        receipt_limit=10,
    )
...
def test_ui_obs2f_rejects_nonlocal_channels():
    report = simulate_local_delivery(
        {"decisions": [_decision("bad", channels=["email", "local_desktop"])]}, delivered_at=NOW
    )
