...
def test_gh1j_identifies_missing_snapshot_as_first_break(tmp_path: Path) -> None:
    ...
    for table, timestamp in (
        ...
        ("market_rankings", "ranked_at"),
        ("market_opportunities", "detected_at"),
        ("advanced_risk_decisions", "decision_timestamp"),
    ):
                ...
                "two_sided_books": [
                    {
                        "ticker": "KXBTC-TEST",
                        "category": "crypto",
                        "calibration": {"ranking_advance": True, "risk_executable_advance": True},
                    }
                ]
            }
        ),
