...
class OwnedRuntimeTests(unittest.TestCase):
    ...
    def args(self, seconds):
        return dict(
            output=self.output,
            cutoff=R.clock() + timedelta(seconds=seconds),
            required_seconds=1,
            maximum_seconds=10,
    ...
    def test_actual_persistence_delay_refuses_without_child_marker(self):
        ...
        with self.assertRaisesRegex(ValueError, "LATE_START_REFUSED"):
            R.run_owned(
                (sys.executable, "-c", "open('UNAUTHORIZED_CHILD','w').write('started')"),
                **args,
                _after_intent=lambda: time.sleep(2.1),
            )
    ...
    def test_actual_overrun_stops_only_owned_child_before_cutoff(self):
        ...
        self.assertEqual(r["status"], "DEADLINE_STOP")
        self.assertLess(r["finished_at"], r["cutoff"])
    ...
    def test_leader_exit_first_still_reaps_owned_descendant(self):
        ...
        self.assertEqual(r["status"], "DESCENDANTS_TERMINATED")
        self.assertEqual(len(r["reaped_descendant_pids"]), 1)
    ...
    def test_term_resistant_descendant_is_killed_when_leader_exits(self):
        code = (
            "import os,signal,time; p=os.fork(); (signal.signal(signal.SIGTERM,sign"
            "al.SIG_IGN),open('ready','w').close(),time.sleep(20)) if p==0 else ([t"
            "ime.sleep(.01) for _ in range(100) if not os.path.exists('ready')],os."
            "_exit(0))"
        )
        r = R.run_owned((sys.executable, "-c", code), **self.args(8))
        self.assertEqual(r["status"], "DESCENDANTS_TERMINATED")
    ...
    def test_actual_child_entry_delay_refuses_before_substantive_work(self):
        ...
        result = subprocess.run(
            (
                ...
                sys.executable,
                "-c",
                "open('UNAUTHORIZED_CHILD','w').close()",
            ),
            cwd=self.tmp.name,
            timeout=5,
        )
        self.assertEqual(result.returncode, 78)
        self.assertFalse((Path(self.tmp.name) / "UNAUTHORIZED_CHILD").exists())
    ...
    def test_zz_new_session_descendant_for_outer_cgroup_cleanup(self):
        ...
        if pid == 0:
            os.setsid()
            marker.write_text(
                json.dumps(dict(pid=os.getpid(), cgroup=Path("/proc/self/cgroup").read_text()))
            )
            time.sleep(20)
            os._exit(0)
