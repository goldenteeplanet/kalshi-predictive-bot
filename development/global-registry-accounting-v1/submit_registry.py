...
source = base / "worktree/development/global-registry-accounting-v1"
attempt = base / "attempt-registry-accounting-v1"
output = base / "results-registry-accounting-v1"
...
def submit(intent):
    ...
    guard = call(
        [
            "systemd-run",
            "--unit=alpha-cloud-registry-cutoff-v1-20260914",
    ...
    launched = call(
        [
            "systemd-run",
            "--unit=alpha-cloud-registry-accounting-v1-20260914",
            *[f"--property={p}" for p in properties],
            "/usr/bin/python3",
            "/work/run_registry.py",
        ]
    )
    D.save(
        attempt,
    ...
    effective = call(
        [
            "systemctl",
            "show",
            "alpha-cloud-registry-accounting-v1-20260914.service",
            "-p",
    ...
    return {"status": "SUBMITTED_ONCE", "effective": effective.stdout}
