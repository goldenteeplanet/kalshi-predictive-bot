...
from capacity_plan import MAX_BYTES, PAGE_PHASE_BYTES, Charge, plan
...
class CapacityPlanTests(unittest.TestCase):
    def call(self, **changes):
        args = dict(
            filesystem="volume-1",
            epoch="epoch-new",
            available_bytes=PAGE_PHASE_BYTES,
            observed_at="2026-09-14T13:00:00Z",
            decision_at="2026-09-14T13:00:01Z",
            max_age_seconds=2,
    ...
    def test_numeric_and_collection_bounds(self):
        for changes in (
            ...
            {"pages": 121},
            {"charges": []},
            {"protected_margin_bytes": -1},
            {"available_bytes": MAX_BYTES + 1},
            {"max_age_seconds": 0},
        ):
            with self.assertRaises(ValueError):
                self.call(**changes)
